...
X = df['numeros'].apply(lambda x: x[:]).tolist()  # Primeiros 14 números como entrada
...
model.add(Dense(units=128, input_dim=15, activation='relu'))
...
model.add(Dense(units=15 * 25, activation='linear'))  # 15 números, cada um com 25 possíveis
...
# Treinando o modelo
model.fit(X_train, y_train_onehot, epochs=50, batch_size=32, validation_data=(X_test, y_test_onehot), verbose=0)
...
ultimo_sorteio = df['numeros'].iloc[-1][:]  # 15 números
ultimo_sorteio_scaled = scaler.transform([ultimo_sorteio])  # Normalizando os dados de entrada
previsao = model.predict(ultimo_sorteio_scaled, verbose=0)
